...
# the dispatch type of a single argument value: a closed type value dispatches on
# egality (`TypeEgal{a}`), matching the runtime argument tuple (see
# `jl_inst_arg_tuple_type`); other values use the ordinary `Core.Typeof`. This keeps
# reflection (`typesof`, hence `@inferred`) consistent with actual dispatch.
...
function descend_params(io::IO, @nospecialize(sig), @nospecialize(called))
    # a type-valued argument may arrive as the egal dispatch kind `TypeEgal{T}`; treat it
    # like the equality `Type{T}` so the diff descends into `T` (JuliaLang/julia#61323)

...
function show_unquoted(io::IO, ex::Expr, indent::Int, prec::Int, quote_level::Int = 0)
    ...
    elseif head === :call && nargs >= 1
        ...
        elseif isa(func,Symbol) && length(func_args) == 1 && func in uni_ops
            show_unquoted(io, func, indent, 0, quote_level)
            arg1 = func_args[1]
            # Parenthesize a negative real literal so it does not glue onto the
            # operator into an unparseable token, e.g. -(-4) printed as --4.
            if isa(arg1, Expr) || (isa(arg1, Symbol) && isoperator(arg1) && is_valid_identifier(arg1)) || (isa(arg1, Real) && signbit(arg1))

...
# Set of LLVM feature names for the host arch; used by @cpu_supports for
...
"""
    Base.@cpu_supports name1 [name2 ...] -> Bool

Compile-time CPU capability query, ANDed across names. Each `nameN` is a
literal naming an LLVM target feature (`avx2`, `fma`, `"sse4.2"`) or a CPU
model (`haswell`, `znver4`, `"x86-64-v3"`). Strings are required for names
that contain `-` or `.`. CPU-model names expand to their JIT-relevant
feature set. Unknown names error at macro-expansion time.

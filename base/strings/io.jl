...
If `io` is given, the result is written to `io` rather than returned. Otherwise the
result is returned as a `String`, unless any item is an `AnnotatedString`, in which
case the result is an `AnnotatedString`.

...
static bool _can_optimize_isa(jl_value_t *type, int &counter)
{
    ...
    if (type == (jl_value_t*)jl_type_type)
        return true;
    // `isa(x, T)` is a pointer comparison only when `T`'s sole inhabitant is a
    // single pointer-unique value, i.e. `T == TypeEgal{X}` (egality) with `X`
    // pointer-egal. `Type{X}` (equality) does not qualify (JuliaLang/julia#61323).
...
static std::pair<Value*, bool> emit_isa(jl_codectx_t &ctx, const jl_cgval_t &x, jl_value_t *type, const Twine &msg)
{
    ...
    if (is_uniquerep_Type(intersected_type) && jl_pointer_egal(intersected_type)) {
        // `TypeEgal{X}` with pointer-egal `X`: `isa(x, T)` is `x === X`, a pointer
        // comparison. `Type{X}` (equality) is excluded (JuliaLang/julia#61323).

                                ...
                                struct recur_list *depth, jl_static_show_config_t ctx) JL_NOTSAFEPOINT
{
    ...
    // exist only by pointer identity in early startup
        ...
        // the egality kind is distinct from (and more specific than) `Type{T}`, so it
        // prints under its own name; qualify it as `Core.TypeEgal` (it is not exported)
        // so the output stays eval-able, e.g. in `--trace-compile` precompile statements

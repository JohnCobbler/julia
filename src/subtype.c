...
// `jl_is_kind` tests only the concrete kinds (`DataType`, `Union`, ...). `AnyType` is
// the abstract kind (`Type === AnyType`): its instances are all types and `AnyType <: Type`.
// Type-relation queries that ask "is `t` a kind" (e.g. is `t <: Type`) must accept it too.

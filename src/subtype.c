...
static int obviously_unequal(jl_value_t *a, jl_value_t *b) JL_NOTSAFEPOINT
{
    ...
    if (jl_is_typeegal(a) && jl_is_typeegal(b))
        // egality: `TypeEgal{A}` and `TypeEgal{B}` are equal iff `A === B`
...
static int subtype(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, jl_param_pos_t param)
{
    ...
    if (jl_is_unionall(y))
        return subtype_unionall(x, (jl_unionall_t*)y, e, 1, param);
    if (jl_is_typeegal(x)) {
        // `TypeEgal{A}`'s only instance is exactly `A` (matched by `===`); `A` is
        // always closed (free typevars are disallowed inside `TypeEgal`).
        jl_value_t *A = jl_typeegal_T(x);
        if (jl_is_typeegal(y))
            // egality: `TypeEgal{A} <: TypeEgal{B}` iff `A === B`
            return jl_types_egal(A, jl_typeegal_T(y));
        if (jl_is_typeeq(y)) {
            // egal implies equal, so reduce to the `Type{A} <: Type{B}` test
            // (type equality), exactly as for `TypeEq{A} <: TypeEq{B}`
            e->invdepth++;
            int ans = forall_exists_equal(A, jl_typeeq_T(y), e);
            e->invdepth--;
            return ans;
        }
        // for any other `y` (e.g. a kind like `DataType`), the single instance `A`
        // dispatches as the singleton type `typeof(A)`
        return subtype(jl_typeof(A), y, e, param);
    }
    if (jl_is_typeegal(y)) {
        // `x` is not a `TypeEgal` here. A single-element set `{B}` contains no other
        // type as a subtype (besides `Union{}`, already handled), so conservatively
        // report not-a-subtype.
        return 0;
    }
...
static int obvious_subtype(jl_value_t *x, jl_value_t *y, jl_value_t *y0, int *subtype)
{
    ...
    if (jl_is_typeegal(x)) {
        jl_value_t *A = jl_typeegal_T(x);
        if (jl_is_typeegal(y)) {
            // egality is decidable here
            *subtype = jl_types_egal(A, jl_typeegal_T(y));
            return 1;
        }
        // for a plain datatype/kind `y`, `A` dispatches as the singleton `typeof(A)`.
    ...
    if (jl_is_typeegal(y)) {
        // nothing but `Union{}` (handled above) and egal `TypeEgal`s is a subtype of `TypeEgal{B}`
        *subtype = 0;
        return 1;
    }
    if (jl_is_uniontype(x)) {
...
JL_DLLEXPORT int jl_isa(jl_value_t *x, jl_value_t *t)
{
    ...
    if (jl_is_type(x)) {
        ...
        if (!jl_has_free_typevars(x)) {
            ...
            if (jl_is_typeegal(t))
                // `isa(x, TypeEgal{A})` holds iff `x === A` (egality)
...
static jl_value_t *intersect(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, jl_param_pos_t param)
{
    ...
    if (jl_is_typeegal(x) || jl_is_typeegal(y)) {
        // `TypeEgal{A}`'s sole inhabitant is exactly `A`, so the intersection is
        // `TypeEgal{A}` itself when `A` lies in the other operand, else `Bottom`.
        // We must return the (more specific) `TypeEgal{A}`, not `Type{A}`, since
        // `Type{A} <: TypeEgal{A}` does not hold. `A` is always closed.
        int8_t R = 0;
        if (!jl_is_typeegal(x)) { jl_value_t *t = x; x = y; y = t; R = 1; }
        jl_value_t *A = jl_typeegal_T(x);
        if (jl_is_typeegal(y))
            // egality: equal iff `A === B`
            return jl_types_egal(A, jl_typeegal_T(y)) ? x : jl_bottom_type;
        if (jl_is_typeeq(y)) {
            // `A ∈ Type{B}` iff `A == B` (binding `B`'s typevars); reuse the
            // invariant `Type{}` machinery for the test/binding, keep `TypeEgal{A}`.
        ...
        // a plain kind/datatype `y`: `A` dispatches as the singleton `typeof(A)`
...
static jl_value_t *insert_nondiagonal(jl_value_t *type, jl_varbinding_t *troot, int widen2ub)
{
    if (jl_is_typevar(type)) {
        ...
        // `T` is closed, so this is a no-op, but keep it parallel to `TypeEq`
...
static int type_morespecific_(jl_value_t *a, jl_value_t *b, jl_value_t *a0, jl_value_t *b0, int invariant, jl_typeenv_t *env)
{
    ...
    if (jl_is_typeegal(a) || jl_is_typeegal(b)) {
        // `TypeEgal{A}` is exactly as specific as `Type{A}` (`TypeEq`); the egality
        // distinction is purely a cache-key concern, so normalize before comparing.

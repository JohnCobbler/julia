...
// should agree with `Core.Compiler.isconstType`
static bool is_uniquerep_Type(jl_value_t *t)
{
    // Only `TypeEgal{T}` (egality) is known to have a unique inhabitant `T` and may
    // be inlined as the constant `T`. `Type{T}` (equality) cannot: for every `T`
    // there is an `S` with `S == T` yet `S !== T`, so the old `type_has_unique_rep`
    // test was unsound (JuliaLang/julia#61323).
...
static inline jl_cgval_t ghostValue(jl_codectx_t &ctx, jl_value_t *typ)
{
    ...
    if (jl_is_some_typeeq(typ)) {
        // `TypeEgal{T}` has the single value `T`; `Type{Union{}}` (the typeofbottom
        // alias) has the single value `Union{}`. Other `Type{T}` are not ghosts
        // (their value is only known up to `== T`; JuliaLang/julia#61323).
...
static const char *derive_sigt_name(jl_value_t *jargty)
{
    ...
    if (jl_is_some_typeeq((jl_value_t*)dt)) {
        dt = (jl_datatype_t*)jl_argument_datatype(jl_some_typeeq_T((jl_value_t*)dt));
        if ((jl_value_t*)dt != jl_nothing) {

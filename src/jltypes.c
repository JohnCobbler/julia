    ...
    for (j = 0; j < n; j++) {
        ...
        if (tj != kj) {
            if (tt->name == jl_tuple_typename) {
                ...
                // `TypeEgal{T}` (egality) gets the same identity-preserving treatment.
...
// hash of `TypeEgal{T}`. Like `typeeq_hash`, shared between hashing by type
// (`type_hash`) and by value (`typekeyvalue_hash`) so the two cannot diverge.
// Since `TypeEgal` membership is egality (and free typevars are disallowed in
// `T`), the hash is purely structural in `T` — no equality normalization — and
// uses a distinct mixing constant so it does not collide with `TypeEq{T}`.
static unsigned typeegal_hash(jl_value_t *T, int *failed) JL_NOTSAFEPOINT
{
    unsigned hashT = type_hash(T, failed);
...
static unsigned typekeyvalue_hash(jl_typename_t *tn, jl_value_t *key1, jl_value_t **key, size_t n, int leaf) JL_NOTSAFEPOINT
{
    ...
    for (j = 0; j < n; j++) {
        ...
        if (leaf && jl_is_kind(jl_typeof(kj))) {
            // a type-valued argument is cached as `TypeEgal{kj}` (egality) when it is
            // closed, or as `Type{kj}` when it has free typevars; hash to match
            // whichever key `jl_inst_arg_tuple_type` stores
            int failed = 0;
        ...
        for (i = 0; i < nargs; i++) {
            ...
            if (leaf && jl_is_type(ai)) {
                if (jl_has_free_typevars(ai)) {
                    // free typevars are disallowed inside `TypeEgal` (and this is not
                    // a concrete value); fall back to the equality key `Type{ai}`,
                    // which still binds static parameters (see issue #61242).
                    ai = (jl_value_t*)jl_wrap_Type(ai);
                }
                else {
                    // specialize the dispatch cache on the type value by egality:
                    // `TypeEgal{ai}` matches exactly `ai` (`===`), so type-equal but
                    // distinct type objects get distinct cache entries; `jl_compilation_sig`
                    // un-egals any slot the matched method pins to a concrete `Type{X}`.
...
static jl_value_t *inst_type_w_(jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack, int check, int nothrow)
{
    ...
    if (jl_is_typeegal(t)) {
        // `T` has no free typevars, so instantiation is a no-op, but recurse for safety
        jl_typeeq_t *te = (jl_typeeq_t*)t;
        jl_value_t *T = inst_type_w_(te->T, env, stack, check, nothrow ? 1 : 0);
        JL_GC_PUSH1(&T);
        if (T == NULL) {
...
void jl_init_types(void) JL_GC_DISABLED
{
    ...
    // Egality-based dual to `TypeEq` (`Type{T}`): `TypeEgal{T}`'s only instance is
    // exactly `T` (matched by `===`), and free typevars are disallowed inside `T`.
    // Used internally for the dispatch-cache specialization on type values.

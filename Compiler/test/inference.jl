...
let nfields_tfunc(@nospecialize xs...) =
    ...
    # `Type{X}` widens (a `==`-but-`!==` rep can have a different field count); the egal
    # dispatch kind `TypeEgal{X}` pins the value to exactly `X`, so it stays a constant.
...
let tuple_tfunc(@nospecialize xs...) =
    ...
    # `Type{X}` widens (its `typeof` is not pinned, #61323); the egal kind `TypeEgal{X}`
    # stays a concrete `DataType` element.
...
# Don't pessimize apply_type to anything worse than Type (or TypeVar).
# A `Type{Union}`-typed head may be a `==`-but-`!==` rep of `Union` (JuliaLang/julia#61323),
# so the head's constructor isn't pinned down; the result widens to the Type/TypeVar floor
# rather than the precise values obtained from a `Const(Union)` head.
...
# A dispatched type-value has the egal type `TypeEgal{T}`; the equality type
# `Type{T}` may be a `==`-but-`!==` rep, so it no longer folds (JuliaLang/julia#61323).

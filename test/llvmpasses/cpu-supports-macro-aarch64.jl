...
# CHECK: ret i8 {{[01]}}
query_neon() = Base.@cpu_supports neon
emit(query_neon)
...
# CHECK-NOT: ijl_cpu_supports
# CHECK: ret i8 {{[01]}}
query_multi() = Base.@cpu_supports neon "fp-armv8"
emit(query_multi)
...
# CHECK-NOT: ijl_cpu_supports
# CHECK: ret i8 {{[01]}}
query_v82a() = Base.@cpu_supports "v8.2a"
emit(query_v82a)
...
# CHECK-NOT: ijl_cpu_supports
# CHECK: ret i8 {{[01]}}
query_a78() = Base.@cpu_supports "cortex-a78"
emit(query_a78)
...
# CHECK-NOT: ijl_cpu_supports
# CHECK-NOT: br i1
@inline dispatch(x) = Base.@cpu_supports(neon) ? x * 2 : x + 1
emit(dispatch, Int)
...
function assert_holds(x::Int)
    @assert Base.@cpu_supports neon
    return x * 2
end
emit(assert_holds, Int)

# Assert false via the __never__ sentinel.
